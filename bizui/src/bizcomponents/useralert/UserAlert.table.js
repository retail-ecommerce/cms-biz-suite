...
import styles from './UserAlert.table.less'
...
class UserAlertTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = UserAlertBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/userAlert/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
