...
import styles from './UserAlert.preference.less'
...
const internalImageListOf = (userAlert) =>{
...
const internalSettingListOf = (userAlert) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = UserAlertService[candidateServiceName] 
...
const executeTrans = (userAlert,targetComponent) =>{
...
const buildTransferModal = (userAlert,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(userAlert,targetComponent)}
...
const internalRenderExtraHeader = (userAlert) =>{
	return null
}
const internalRenderExtraFooter = (userAlert) =>{
	return null
}
...
<Description term="Profile">{userAlert.profile==null?appLocaleName(userContext,"NotAssigned"):userAlert.profile.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Profile","profile","requestCandidateProfile",
	      "transferToAnotherProfile","anotherProfileId",userAlert.profile?userAlert.profile.id:"")} 
...
const renderPermissionSetting = userAlert => {
  const {UserAlertBase} = GlobalComponents
  return <PermissionSetting targetObject={userAlert}  targetObjectMeta={UserAlertBase}/>


}


class UserAlertPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"User Alert",cardsFor: "userAlert",cardsSource: this.props.userAlert,
  		subItems: [
...
}))(Form.create()(UserAlertPreference))
