...
import styles from './Target.preference.less'
...
const internalImageListOf = (target) =>{
...
const internalSettingListOf = (target) =>{
	const userContext = null
	const optionList = [ 
	]
...
const internalLargeTextOf = (target) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {TargetService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = TargetService[candidateServiceName] 
...
const executeTrans = (target,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_target/doJob",payload: payload})
...
const buildTransferModal = (target,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(target,targetComponent)}
...
const internalRenderExtraHeader = (target) =>{
	return null
}
const internalRenderExtraFooter = (target) =>{
	return null
}
...
const internalSummaryOf = (target,targetComponent) =>{
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="Id">{target.id}</Description> 
<Description term="Name">{target.name}</Description> 
<Description term="Profile">{target.profile==null?appLocaleName(userContext,"NotAssigned"):target.profile.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Profile","profile","requestCandidateProfile",
	      "transferToAnotherProfile","anotherProfileId",target.profile?target.profile.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="Banner">{target.banner==null?appLocaleName(userContext,"NotAssigned"):target.banner.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"Banner","banner","requestCandidateBanner",
	      "transferToAnotherBanner","anotherBannerId",target.banner?target.banner.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
const renderPermissionSetting = target => {
  const {TargetBase} = GlobalComponents
  return <PermissionSetting targetObject={target}  targetObjectMeta={TargetBase}/>


}


class TargetPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.target
    const cardsData = {cardsName:"Target",cardsFor: "target",cardsSource: this.props.target,
  		subItems: [
