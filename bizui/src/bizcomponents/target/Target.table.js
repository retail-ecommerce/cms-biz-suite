...
import styles from './Target.table.less'
...
class TargetTable extends PureComponent {
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = TargetBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/target/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
