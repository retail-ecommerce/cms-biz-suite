...
import styles from './Banner.preference.less'
...
const internalImageListOf = (banner) =>{
...
const internalSettingListOf = (banner) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = BannerService[candidateServiceName] 
...
const executeTrans = (banner,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_banner/doJob",payload: payload})
...
const buildTransferModal = (banner,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(banner,targetComponent)}
...
const internalRenderExtraHeader = (banner) =>{
	return null
}
const internalRenderExtraFooter = (banner) =>{
	return null
}
...
const renderPermissionSetting = banner => {
  const {BannerBase} = GlobalComponents
  return <PermissionSetting targetObject={banner}  targetObjectMeta={BannerBase}/>


}


class BannerPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, targetCount } = this.props.banner
    const cardsData = {cardsName:"Banner",cardsFor: "banner",cardsSource: this.props.banner,
  		subItems: [
