...
import styles from './AlertBar.preference.less'
...
const internalImageListOf = (alertBar) =>{
...
const internalSettingListOf = (alertBar) =>{
	const userContext = null
	const optionList = [ 
	]
...
const internalLargeTextOf = (alertBar) =>{

	return(<div> 
   <Card title={`Message`} ><pre>{alertBar.message}</pre></Card>
</div>)
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {AlertBarService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = AlertBarService[candidateServiceName] 
...
const executeTrans = (alertBar,targetComponent) =>{
...
const buildTransferModal = (alertBar,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(alertBar,targetComponent)}
...
const internalRenderExtraHeader = (alertBar) =>{
	return null
}
const internalRenderExtraFooter = (alertBar) =>{
	return null
}
...
const internalSummaryOf = (alertBar,targetComponent) =>{
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="Id">{alertBar.id}</Description> 
<Description term="Name">{alertBar.name}</Description> 
<Description term="Last Update">{ moment(alertBar.lastUpdate).format('YYYY-MM-DD')}</Description> 
	
        {buildTransferModal(alertBar,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = alertBar => {
  const {AlertBarBase} = GlobalComponents
  return <PermissionSetting targetObject={alertBar}  targetObjectMeta={AlertBarBase}/>


}


class AlertBarPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.alertBar
    const cardsData = {cardsName:"Alert Bar",cardsFor: "alertBar",cardsSource: this.props.alertBar,
  		subItems: [
...
export default connect(state => ({
  alertBar: state._alertBar,
}))(Form.create()(AlertBarPreference))
